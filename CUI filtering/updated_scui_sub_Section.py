...
# ----------------------------
# Step 5: Load UMLS hierarchy – bring ancestors for fetched CUIs
# ----------------------------
class MRRELCache:
...
# ----------------------------
# Step 6: Bring descendant count for fetched CUIs
# ----------------------------
def compute_ic(cuis, descendants_table, project_id, dataset):
    # Fetch descendant counts and compute IC using only NER CUIs as denominator
    if not cuis:
        return {}, 0
    ...
    # ----------------------------
    # Step 7: Calculate IC and median threshold
    # ----------------------------
...
# ----------------------------
# Step 8: Get all ancestors for a given CUI
# ----------------------------
...
# ----------------------------
# Step 9: Apply IC and median threshold to ancestors and filter CUIs
# ----------------------------
def semantic_rollup(text, ner_url, mrrel_cache, descendants_table, project_id, dataset):
     # Step 9a: Get CUIs from NER API
    cuis = get_cuis_from_text(text, ner_url)
    if not cuis:
        print("[Rollup] No CUIs found for text.")
        return pd.DataFrame()
    
    # Step 9b: Load UMLS hierarchy
    mrrel_cache.load()
    
    # Step 9c: Expand to ancestors
    all_cuis = set(cuis)
    for cui in cuis:
        all_cuis.update(get_ancestors(cui, mrrel_cache.parent_map))
    
    # Step 9d: Compute IC for all CUIs (NER + ancestors)
    ic_map, threshold = compute_ic(list(all_cuis), descendants_table, project_id, dataset)
    if not ic_map:
        print("[Rollup] No IC computed.")
        return pd.DataFrame()
    
    # Step 9e: Filter CUIs with IC >= threshold
    final_cuis = {cui for cui in all_cuis if ic_map.get(cui, 0) >= threshold}


# ----------------------------
# Step 10: Example usage and save output
# ----------------------------
