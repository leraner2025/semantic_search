...
class EnhancedCUIReducer:
    ...
    def reduce(
        ...
        if use_semantic_clustering:
            final_cuis = self._semantic_clustering_safe(rolled_up, hierarchy, ic_scores)
    ...
    def _compute_ic_scores_safe(self, hierarchy: Dict) -> Dict[str, float]:
        ...
        ic_scores = {cui: max(0.0, -np.log((count_desc(cui) + 1) / total)) for cui in all_cuis}
    ...
    def _semantic_clustering_safe(
        ...
        hierarchy: Dict,
        ic_scores: Dict[str, float],
        distance_percentile: float = 25.0
    ) -> List[str]:
        ...
        # ----------------------------
        # Compute all pairwise distances in hierarchy
        # ----------------------------
        def hierarchy_distance(c1, c2):
            # BFS to find shortest distance via common ancestor
            if c1 == c2:
                return 0
            visited = set()
            queue = deque([(c1, 0)])
        ...
        for i in range(n):
            for j in range(i + 1, n):
                d = hierarchy_distance(all_cuis[i], all_cuis[j])
                dist_matrix[i, j] = d
                dist_matrix[j, i] = d
        ...
        # Dynamic threshold
        # ----------------------------
        tri_vals = dist_matrix[np.triu_indices(n, k=1)]
        dist_thr = np.percentile(tri_vals, distance_percentile)
        logger.info(f"Hierarchy-based clustering distance threshold: {dist_thr:.2f}")
        ...
        for i, cui in enumerate(all_cuis):
            ...
            assigned.add(cui)
            for j in range(i + 1, n):
                if all_cuis[j] not in assigned and dist_matrix[i, j] <= dist_thr:
                    cluster.append(all_cuis[j])
                    assigned.add(all_cuis[j])
            clusters.append(cluster)

        # ----------------------------
        # Prune within clusters using IC and distance
        # ----------------------------
        final = []
        for cluster in clusters:
            ...
            idx_map = {c: i for i, c in enumerate(all_cuis)}
            cluster_distances = dist_matrix[
                np.ix_([idx_map[c] for c in cluster], [idx_map[c] for c in cluster])
            ]
            prune_thr = np.percentile(
                cluster_distances[np.triu_indices(len(cluster), k=1)], distance_percentile
            )
            # keep highest IC nodes, prune if too close
            sorted_cluster = sorted(cluster, key=lambda x: ic_scores.get(x, 0), reverse=True)
            kept = []
            for c in sorted_cluster:
                i = cluster.index(c)
                if all(cluster_distances[i, cluster.index(k)] >= prune_thr for k in kept):
                    kept.append(c)
            final.extend(kept)
