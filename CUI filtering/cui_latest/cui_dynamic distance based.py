...
class EnhancedCUIReducer:    
    ...
    def _semantic_clustering_safe(
        ...
        """
        Clusters CUIs and prunes close CUIs within clusters.
        Similarity thresholds and distance thresholds are dynamically computed.
        """
        if len(cui_list) <= 1:
            return cui_list
        ...
        try:
            ...
            # ---- STEP 0: Compute dynamic similarity threshold ----
            all_pairwise_sim = 1 - cosine_distances(embeddings)
            # Flatten upper triangle only
            triu_idx = np.triu_indices_from(all_pairwise_sim, k=1)
            sim_values = all_pairwise_sim[triu_idx]
            similarity_threshold = np.percentile(sim_values, base_similarity_percentile)
            logger.info(f"Dynamic similarity threshold (clustering) set to: {similarity_threshold:.3f}")

            # Convert to distance for AgglomerativeClustering
            ...
            for cluster_id in np.unique(labels):
                ...
                intra_cluster_distances = dist_matrix[triu_idx]
                if len(intra_cluster_distances) > 0:
                    min_intra_cluster_distance = np.percentile(intra_cluster_distances, base_distance_percentile)
                else:
                    min_intra_cluster_distance = 0.25  # fallback
                logger.debug(f"Cluster {cluster_id}: dynamic pruning distance = {min_intra_cluster_distance:.3f}")

                # Sort CUIs by IC DESC (prefer more specific concepts)
                order = sorted(
