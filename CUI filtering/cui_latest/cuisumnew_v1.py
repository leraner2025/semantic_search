...
class EnhancedCUIReducer:
    ...
    def _build_hierarchy_depthwise(self, cuis: List[str]) -> Dict:
        ...
        logger.info(f"Fetching hierarchy for {len(cuis)} CUIs")
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            async def fetch_batch(batch):
                async with session.post(
                    ...
                    if resp.status != 200:
                        logger.warning(f"Batch failed: {resp.status}")
                        return [], []
    ...
    def _semantic_rollup(self, cui_list, hierarchy, ic_scores, ic_threshold):
        ...
        for cui in cui_list:
            ...
            while queue and len(visited) < MAX_NODES:
                ...
                if curr in visited or depth >= self.max_hierarchy_depth:
                    continue
                visited.add(curr)
                neighbors = child_to_parents.get(curr, []) + parent_to_children.get(curr, [])
                for n in neighbors:
                    if n not in visited:
                        candidates.add(n)
            ...
            # choose lowest-IC = most general OR most connected
            rolled_up[cui] = min(
                valid,
                key=lambda c: ic_scores.get(c, 0),
                default=cui
            )
    ...
    min_intra_cluster_distance: float = 0.25
        ...
        try:
            ...
            clustering = AgglomerativeClustering(
                n_clusters=None,
                distance_threshold=1 - perc95,  # percentile-based threshold
                metric='cosine',
                linkage='average'
            )
